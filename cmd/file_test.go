...
func TestFileListRun(t *testing.T) {
	ctx := testDMContext(func(req *http.Request) (*http.Response, error) {
		switch req.URL.Path {
		case "/api/files.list":
			return dmJSONResponse(req, `{"ok":true,"files":[{"id":"F123","name":"report.txt","title":"Weekly Report","size":1536,"pretty_type":"Plain Text"}]}`)
		default:
...
func TestFileInfoRun(t *testing.T) {
	ctx := testDMContext(func(req *http.Request) (*http.Response, error) {
		switch req.URL.Path {
		case "/api/files.info":
			return dmJSONResponse(req, `{"ok":true,"file":{"id":"F123","name":"report.txt","title":"Weekly Report","size":1536,"pretty_type":"Plain Text","mode":"hosted","created":1775840563,"user":"U123","is_public":false,"file_access":"visible","permalink":"https://example.slack.com/files/F123"}}`)
		default:
			return nil, fmt.Errorf("unexpected path %s", req.URL.Path)
		}
	})
...
func TestFileDownloadRun(t *testing.T) {
	...
	ctx := testDMContext(func(req *http.Request) (*http.Response, error) {
		switch req.URL.Path {
		case "/api/files.info":
			return dmJSONResponse(req, `{"ok":true,"file":{"id":"F123","name":"report.txt","size":5,"url_private_download":"https://files.slack.com/download/F123"}}`)
		case "/download/F123":
...
func TestFileUploadRun(t *testing.T) {
	...
	ctx := testDMContext(func(req *http.Request) (*http.Response, error) {
		...
		case "/api/files.getUploadURLExternal":
			...
			return dmJSONResponse(req, `{"ok":true,"upload_url":"https://upload.example/F123","file_id":"F123"}`)
		case "/api/files.completeUploadExternal":
			...
			return dmJSONResponse(req, `{"ok":true,"files":[{"id":"F123","name":"report.txt","title":"Weekly Report"}]}`)
		case "/F123":
