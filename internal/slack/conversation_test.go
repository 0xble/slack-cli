...
func TestResolveConversationTargetEnterpriseUserID(t *testing.T) {
	...
	if target.ChannelID != "D123" || !target.IsDM || target.Type != "dm" || target.Username != "alice" {
...
func TestResolveConversationTarget(t *testing.T) {
	t.Run("resolves channel id", func(t *testing.T) {
		client := testConversationClient(func(req *http.Request) (*http.Response, error) {
			switch req.URL.Path {
		...
		if target.ChannelID != "C123" || target.ChannelName != "general" || target.Name != "general" || target.Type != "channel" {
			t.Fatalf("unexpected target: %+v", target)
		}
	})
	...
	t.Run("resolves private channel id", func(t *testing.T) {
		...
		if target.ChannelID != "G123" || target.Type != "group" || !target.IsPrivate {
			t.Fatalf("unexpected target: %+v", target)
		}
	})
...
func testConversationClient(fn func(req *http.Request) (*http.Response, error)) *Client {
	return &Client{
		userToken: "xoxp-test-token",
		httpClient: &http.Client{
